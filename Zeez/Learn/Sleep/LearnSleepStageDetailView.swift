...
struct StageComparisonDetailView: View {
        ...
        NavigationView {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("Done") { dismiss() }
                    .accessibilityLabel("Close stage comparison details")
                    .accessibilityHint("Return to stage comparison view")
                    .accessibilityIdentifier("closeStageDetailButton")
                }
            }
        }
    }
    ...
    private var factorHeader: some View {
        ...
        .accessibilityLabel("\(metric.rawValue). \(metric.description)")
    ...
    private var functionSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Primary Functions")
                .font(.headline)
                .accessibilityLabel("Primary functions section")
            
            ForEach(getFunctions(), id: \.self) { item in
                Label(item.text, systemImage: item.icon)
                    .font(.subheadline)
                    .accessibilityLabel("Function: \(item.text)")
            }
        }
        .accessibilityElement(children: .combine)
        .accessibilityIdentifier("functionSection")
    }
    ...
    private var benefitsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Health Benefits")
                .font(.headline)
                .accessibilityLabel("Health benefits section")
