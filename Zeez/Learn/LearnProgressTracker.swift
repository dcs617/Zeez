...
class LearnProgressTracker: ObservableObject {
    ...
    private func updateProgress(for article: LearnArticle, in context: NSManagedObjectContext) {
        ...
        if let startDate = progress.lastReadDate,
           Date().timeIntervalSince(startDate) > TimeInterval(Double(article.readTimeMinutes) * 60.0 * AppConstants.Learning.readingProgressThreshold) {
        ...
        do {
            ...
            ZeezLogger.error(ZeezLogger.learning, "Error saving progress", error: error)
        }
    }
    ...
    func markComplete(article: LearnArticle, in context: NSManagedObjectContext) {
        ...
        do {
            ...
            ZeezLogger.error(ZeezLogger.learning, "Error marking article complete", error: error)
        }
    }
