...
struct LearnArticleShareSheet: View {
    ...
    private func getTextContent() -> String {
        ...
        if includeHighlights, let highlights = article.highlights {
            content += "\nHighlights:\n"
            content += "- \(highlights.text ?? "")\n"
        }
        
        if includeNotes, let notes = article.notes {
            content += "\nNotes:\n"
            content += """
                - \(notes.title ?? "")
                  \(notes.content ?? "")
    ...
    func createPDF() -> Data {
        ...
        return renderer.pdfData { context in
            ...
            if includeHighlights, let highlights = article.highlights {
                ...
                y = drawText("• \(highlights.text ?? "")", at: y, font: .systemFont(ofSize: 12), padding: padding + 20, pageRect: pageRect, context: context)
                y += 10
                y += 20
            }
            ...
            if includeNotes, let notes = article.notes {
                ...
                y = drawText(notes.title ?? "", at: y, font: .boldSystemFont(ofSize: 12), padding: padding + 20, pageRect: pageRect, context: context)
                y += 10
                y = drawText(notes.content ?? "", at: y, font: .systemFont(ofSize: 12), padding: padding + 20, pageRect: pageRect, context: context)
                y += 20
            }
