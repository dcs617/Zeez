...
class EnvironmentalAnalyzer {
    ...
    func analyzeSession(_ session: SleepSession) async throws -> EnvironmentalAnalysis {
        ...
        let temperature = calculateMetrics(readings.compactMap { $0.temperature })
        let humidity = calculateMetrics(readings.compactMap { $0.humidity })
        let noise = calculateMetrics(readings.compactMap { $0.noiseLevel })
        let light = calculateMetrics(readings.compactMap { $0.lightLevel })
