...
struct EnvironmentalDetailView: View {
    ...
    private var factorInfo: some View {
        ...
        .accessibilityLabel("\(factor.rawValue) information: Optimal range \(formatValue(factor.optimalRange.min)) to \(formatValue(factor.optimalRange.max)) \(factor.unit)")
        .accessibilityHint("Environmental factor details and recommendations")
        .accessibilityIdentifier("environmentalFactorInfo")
